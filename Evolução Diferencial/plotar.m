...
% Ex.: lb = [-2 -2] e ub = [2 2]
% g = geracao atual
% P = matriz P de solucoes candidatas; cada indivÃ­duo ocupa uma coluna de P
...
% Ex. jP = [1.9132    0.6394   -5.7512   -1.4110    0.3097    0.9379    2.0521]     
    ...
    if (s == 2),
        ...
        Y = lb(2):1/2:ub(2);
        [x,y] = meshgrid (X, Y);    
        z = AvaliaFitness(fobj,x,y);
        contour(x,y,z);
%       surf(x,y,z); %plota superficie
        hold on    
        plot(P(1,:), P(2,:), 'b*');
        title('Função Otimizada- Evolução Diferencial')
        xlabel('x_1')
        ylabel('x_2')
        grid on
        hold off
